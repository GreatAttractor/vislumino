...
use crate::img_seq::{ImageSequence, ImgSeqError};
use ga_image::{Image, PixelFormat};
use std::fs::{File, OpenOptions};
use std::io::{Read, Seek, SeekFrom};
use std::mem::size_of;

pub fn open_ser_video(path: &std::path::Path) -> Result<Box<dyn ImageSequence>, Box<dyn std::error::Error>> {
...
fn get_ser_color_fmt(color_id: u32) -> Result<SerColorFormat, Box<dyn std::error::Error>> {
...
impl SerVideo {
    fn open(file_name: &std::path::Path) -> Result<SerVideo, Box<dyn std::error::Error>> {
